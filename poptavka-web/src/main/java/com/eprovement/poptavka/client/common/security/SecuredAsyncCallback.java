...
//                onAccessDenied();
                displayError(Response.SC_FORBIDDEN);
                return;
            }
            onServiceFailure(caught, 0);
            return;
        }
...
//            displayError(Response.SC_UNAUTHORIZED);
            return;
        }
        // If access is denied, display an error message box (AccessDeniedBox) - HTTP 403
...
//            new SecurityDialogBoxes.AlertBox(MSGS.pageNotFound()).show();
            displayError(Response.SC_NOT_FOUND);
            return;
        }
...
//            new SecurityDialogBoxes.AlertBox(MSGS.badRequest()).show();
            displayError(Response.SC_BAD_REQUEST);
            return;
        }
...
//            new SecurityDialogBoxes.AlertBox(MSGS.internalError()).show();
            displayError(Response.SC_INTERNAL_SERVER_ERROR);
            return;
        }
...
//            new SecurityDialogBoxes.AlertBox(MSGS.serviceUnavailable()).show();
            displayError(Response.SC_SERVICE_UNAVAILABLE);
            return;
        }
        // Other HTTP errors
        onServiceFailure(caught, errorResponse);
    }
     ...
     * @param caught
     */
    protected void onServiceFailure(Throwable caught, int errorResponse) {
