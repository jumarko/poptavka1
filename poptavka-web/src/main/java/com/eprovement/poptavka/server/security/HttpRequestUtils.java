...
public final class HttpRequestUtils {
    ...
    private static final Log HTTP_UTILS_LOGGER = LogFactory.getLog(HttpRequestUtils.class);
     ...
     * @param headerNameStartWith the string a header should start with. Can't
     * be null.
     * @return true or false
     */
    @SuppressWarnings("unchecked")
    public static boolean containsHeaderStartingWith(final HttpServletRequest request,
            final String headerNameStartWith) {
        if (headerNameStartWith == null) {
            throw new IllegalArgumentException("headerNameStartWith must not be null");
        }

        if (HTTP_UTILS_LOGGER.isDebugEnabled()) {
            HTTP_UTILS_LOGGER.debug("HttpRequestUtils looking for some specific headers...");
        }
        ...
        while (headers.hasMoreElements()) {
            ...
            if (HTTP_UTILS_LOGGER.isDebugEnabled()) {
                HTTP_UTILS_LOGGER.debug("  -> header: " + header);
            }
